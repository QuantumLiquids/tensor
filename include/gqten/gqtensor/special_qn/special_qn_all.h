// SPDX-License-Identifier: LGPL-3.0-only
/*
* Author: Hao-Xin Wang <wanghx18@mails.tsinghua.edu.cn>
* Creation Date: 20201-9-28
*
* Description: GraceQ/tensor project. Special QN class all header.
*/

#ifndef GQTEN_GQTENSOR_SPECIAL_QN_SPECIAL_QN_ALL_H
#define GQTEN_GQTENSOR_SPECIAL_QN_SPECIAL_QN_ALL_H

#include "gqten/gqtensor/special_qn/u1qn.h"
#include "gqten/gqtensor/special_qn/u1u1qn.h"
#include "gqten/gqtensor/special_qn/u1u1znqn.h"

#endif //GQTEN_GQTENSOR_SPECIAL_QN_SPECIAL_QN_ALL_H
